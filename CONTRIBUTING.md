...
Falco projects have a [Code of Conduct](CODE_OF_CONDUCT.md) to which all contributors must adhere.
...
[Join the **mailing list**](https://lists.cncf.io/g/cncf-falco-dev/) for news, announcements, and a Google calendar invite
for our Falco open source meetings.

We also run weekly **community calls** which are open calls to discuss Falco projects from an user perspective.
...
* Enhancement: would you like a new feature/rule to be added to Falco? This is the kind of issue you'll need then.
...
Each rule change must be on its own commit, if a change to a macro is done while changing a rule they can go together
but only one rule per commit must happen.
